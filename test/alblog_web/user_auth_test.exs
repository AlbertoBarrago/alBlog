defmodule AlblogWeb.UserAuthTest do
  use AlblogWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Alblog.Accounts
  alias Alblog.Accounts.Scope
  alias AlblogWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AlblogWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AlblogWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AlblogWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
