defmodule SimpleCrud.Blog do
  ...
  def subscribe_articles(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(SimpleCrud.PubSub, "user:#{key}:articles")
  end

  defp broadcast_article(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(SimpleCrud.PubSub, "user:#{key}:articles", message)
  end
